...
import { rolesAPI, kullanicilarAPI } from "./api";
...
export default function AyarlarStokRBAC({ currentUser, isRootAdmin = false, isBackdoor = false }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setRoles(dbRoles || []);
        const dbUsers = await kullanicilarAPI.getAll();
        setUsers(dbUsers || []);
      } catch (error) {
  ...
  async function saveUserModal() {
    ...
    try {
      if (userModal.addMode) {
        ...
        await kullanicilarAPI.update(filteredUsers[userModal.idx].id, updateObj);
      }
  ...
  async function confirmDeleteUser() {
    ...
    try {
      await kullanicilarAPI.delete(id);
  ...
  async function saveRoleModal() {
    ...
    try {
      if (roleModal.addMode) {
        ...
        await rolesAPI.update(name, { label, color, permissions: permState });
      }
