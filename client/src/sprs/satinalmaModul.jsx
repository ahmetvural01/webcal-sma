...
import { getSiparisler as apiGetSiparisler, getLastSiparisNumber as apiGetLastSiparisNumber, createSiparis as apiCreateSiparis, updateSiparis as apiUpdateSiparis, deleteSiparis as apiDeleteSiparis, getTedarikciler as apiGetTedarikciler, createTedarikci as apiCreateTedarikci } from "../api";
...
export default function SatinalmaModul({ user }) {
  ...
  useEffect(() => {
    async function fetchTedarikciler() {
      try {
        const data = await apiGetTedarikciler();
  ...
  useEffect(() => {
    async function yeniNo() {
      try {
        const { lastNumber } = await apiGetLastSiparisNumber();
        setSiparisNo(generateSiparisNo(lastNumber));
      } catch (error) {
  ...
  async function fetchSiparisler() {
    setLoading(true);
    try {
      const data = await apiGetSiparisler(filtre);
      setSiparisler(data || []);
    } catch (error) {
  ...
  async function handleTedarikciEkle() {
    ...
    try {
      const data = await apiCreateTedarikci(yeniTedarikci);
  ...
  async function handleSiparisKaydet() {
    ...
    try {
      ...
      const data = await apiCreateSiparis({
        siparis_no: siparisNo,
        tedarikci_adi: tedarikciObj ? tedarikciObj.adi : "",
        aciklama,
        urunler,
        durum: "beklemede",
        olusturan_kullanici: user.kullanici_adi
      });
